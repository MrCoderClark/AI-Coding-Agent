...
def _truncate_by_lines(text: str, target_tokens: int, suffix: str, model):
    ...
    if not result_lines:
        # Fall back to character truncation if not complete lines fit
        return _truncate_by_chars(text, target_tokens, suffix, model)

    return "\n".join(result_lines) + suffix


def _truncate_by_chars(text: str, target_tokens: int, suffix: str, model):
