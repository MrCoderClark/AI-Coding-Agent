...
def _parse_toml(path: Path):
    try:
        ...
        raise ConfigError(
            f"Error reading config file {path}: {e}", config_file=str(path)
        ) from e
...
def _meerge_dicts(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    result = base.copy()
    for key, value in override.items():
        if key in result and isinstance(result[key], dict) and isinstance(value, dict):
            result[key] = _meerge_dicts(result[key], value)
        else:
            result[key] = value
...
def load_config(cwd: Path | None) -> Config:
    ...
    if project_path:
        try:
            ...
            config_dict = _meerge_dicts(config_dict, project_config_dict)
        except ConfigError as e:
