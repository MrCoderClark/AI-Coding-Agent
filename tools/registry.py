...
class ToolRegistry:
    ...
    async def invoke(
        ...
        validatation_errors = tool.validate_params(params)
        if validatation_errors:
            return ToolResult.error_result(
                f"Invalid parameters: {'; '.join(validatation_errors)}",
                metadata={
                    "tool_name": name,
                    "validatation_errors": validatation_errors,
                },
            )
